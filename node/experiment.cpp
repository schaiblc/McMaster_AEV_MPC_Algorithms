...
private:
    ...
    std::string odom_frame, base_frame, base_frame_imu;
...
RacecarExperiment() {
    ...
    n.getParam("base_frame_imu", base_frame_imu);
    ...
    odom_imu_transform.child_frame_id = base_frame_imu;
   ...
   // Start a subscriber to listen to mux messages
    ...
    void vesc_callback(const vesc_msgs::VescStateStamped & state){
       ...
       if (!waiting_on_init_yaw)
       {
        ...
        odom.pose.pose.orientation.z = odom_imu.pose.pose.orientation.z;
        odom.pose.pose.orientation.w = odom_imu.pose.pose.orientation.w;
      ...
      // Publish odometry tranformation
        odom_transform.transform.translation.x= x_odom_imu;
        odom_transform.transform.translation.y= y_odom_imu;
        odom_transform.transform.translation.z= 0;
        geometry_msgs::Quaternion odom_imu_quat = tf::createQuaternionMsgFromYaw(yaw_imu);
        odom_transform.transform.rotation = odom_imu_quat;

        odom_imu_transform.header.stamp=state.header.stamp;
        odom_broadcaster.sendTransform(odom_transform);
        //Modified to provide the IMU dta in base_link tf, hopefully reduces odom inaccuracy
      }
